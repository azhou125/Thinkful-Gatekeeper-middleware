...
function gateKeeper(req, res, next) {
  ...
  const user_and_password=req.get('x-username-and-password');
  const user_and_password_obj=queryString.parse(user_and_password);
  const find = USERS.find(function(elt){
    return (elt.userName===user_and_password_obj.user && elt.password===user_and_password_obj.pass);
  });
  console.log(find);
  req.user=find;
